...
public:
  ...
  void produce(edm::StreamID sid, device::Event& event, device::EventSetup const& setup) const override{
    ...
    auto mc = MultiCollection();

    mc.push_back(edm::RefProd<SoADeviceCollection>(&soaInputHandle1));
    mc.push_back(edm::RefProd<SoADeviceCollection>(&soaInputHandle2));
    mc.push_back(edm::RefProd<SoADeviceCollection>(&soaInputHandle3));

    // Move the SoA Collection manager into the Event.
    event.emplace(outputToken_, std::move(mc));
  }
...
private:
  ...
  const edm::EDPutTokenT<MultiCollection> outputToken_;
};
