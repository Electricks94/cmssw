...
TEST_CASE("Test MultiView") {
  ...
  for (auto const& device : cms::alpakatools::devices<Platform>()) {
    ...
    SECTION("Host MultiView") {
      auto mv = MultiView<SoAConstView>(soav1, soav2, soav3, soav4);
      REQUIRE(mv.size() == elements1 + elements2 + elements3 + elements4);

      // Loop over all elements of the four views
      for (std::size_t local_idx = 0; local_idx < mv.size(); ++local_idx) {
        auto localView = mv.getView(local_idx);
        const float scalar = static_cast<float>(localView.x2());
        auto slvi = mv[local_idx];
    ...
    SECTION("Device MultiView") {
      ...
      auto devicemv = MultiView<SoAConstView>(deviceCollection1.view(), deviceCollection2.view());
      REQUIRE(devicemv.size() == totalSize);

      auto blockSize = 64;
      auto numberOfBlocks = cms::alpakatools::divide_up_by(devicemv.size(), blockSize);
      const auto workDiv = cms::alpakatools::make_workdiv<Acc1D>(numberOfBlocks, blockSize);

      alpaka::exec<Acc1D>(queue, workDiv, ConsumerKernel{}, devicemv, d_result);
