...
TEST_CASE("test shrinkToFit soa alpaka", "[ShrinkToFit][Alpaka]") {
  ...
  for (auto const& device : devices) {
    ...
    for (int i = 0; i < hostCollection.size(); i++) {
      ...
      h_view1[i].matrix().setConstant(5.0);
      h_view1[i].candidateDirection().setConstant(6.f);
    }
    ...
    for (int i = 0; i < n2; i++) {
        REQUIRE(h_viewShrinked[i].eigenvalues() == Catch::Approx(h_view1[i].eigenvalues()));
        REQUIRE(h_viewShrinked[i].eigenvector_1() == Catch::Approx(h_view1[i].eigenvector_1()));
        REQUIRE(h_viewShrinked[i].eigenvector_2() == Catch::Approx(h_view1[i].eigenvector_2()));
        REQUIRE(h_viewShrinked[i].eigenvector_3() == Catch::Approx(h_view1[i].eigenvector_3()));
        REQUIRE(h_viewShrinked[i].matrix() == h_view1[i].matrix());
        REQUIRE(h_viewShrinked[i].candidateDirection() == h_view1[i].candidateDirection());
    }*/
  }
}